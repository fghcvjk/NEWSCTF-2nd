...
<?php if(!empty($encrypt_request)){ ?>
    (function(){
        ...
        if (!isMobile()) {
            ...
            document.write('<script src="/index.php/request/'+hash+'" type="text/javascript" charset="utf-8">\<\/script>');
            <?php }?>
    ...
    if(hash){
        ...
        if (is_auth) {
            ...
            $('#next').click(function () {
                ...
                $.get('/index.php/request/' + hash, {password: password}, function (ret) {
                    if (ret.code == 200) {
                        let a = eval(ret.data);
                        a = JSON.parse(a);
                        // 验证后的操作，富文本、直接跳转啦。
            ...
            // 富文本
            $.get('/index.php/request/' + hash, {}, function (ret) {
                if (ret.code == 200) {
